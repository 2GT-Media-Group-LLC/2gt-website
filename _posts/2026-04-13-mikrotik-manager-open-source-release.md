...
## Enabling the RouterOS API on Your Devices

If you haven't done this already, SSH into each MikroTik device you want to manage and run:

```
/ip service enable api
```

For API over SSL on port 8729:
```
/ip service enable api-ssl
```

The default port is `8728`. You can configure a custom port per device inside MikroTik Manager if yours is different.
